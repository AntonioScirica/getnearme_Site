// PED template renderer (server-side).
//
// Historically this file mirrored the template HTML builders that lived in
// src/app/metrics/social/page.tsx, with an "IMPORTANT: keep in sync" comment.
// That duplication is gone: the builders now live in a single TypeScript
// registry at ./builders/ and are imported by BOTH the client and this server
// path. Change a builder once → it changes everywhere.
//
// What stays local to this file:
//   - PED_CSS / STORY_CSS  → the server-specific stylesheets (they intentionally
//     differ from the client's: amber accents, neo-brutalist shadows). The cron
//     renders PNGs for Instagram with these.
//   - wrapHtml             → Puppeteer-only document wrapper.
//
// Everything else is re-exported from ./builders so existing consumers
// (cron/generate-ped.js, scripts/send-template-previews.mjs) keep working
// with zero import changes.

// ── Server-specific stylesheets (kept verbatim from the original file) ──
...
// ── Re-export everything from the shared registry ────────────────────
// Plain .js consumers (cron) import these names; the registry is TS but Next
// transpiles it on both runtimes, so `import { ... } from './builders'` works.
export {
  buildSlidesForTopic,
  buildStoryForTopic,
  buildCaptionForTopic,
  storyTeaserD,
  PED_EDU_ITEMS,
  REF_STEPS,
  RUBRIC_STORY,
