...
export function VideoCutsEditor({
  ...
  const stepStyle = s('display:flex;align-items:center;gap:10px;font-size:14px;color:#57534c;line-height:1.4');
  const numBadge = (n: number) => (
    <span style={{ flex: 'none', width: 24, height: 24, borderRadius: '50%', background: ACCENT, color: '#fff', fontSize: 13, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{n}</span>
  );

  const TransformBtn = (
    <Box as="button" onClick={onTransform} disabled={picking || detecting} style={s(`width:100%;border:none;background:${ACCENT};color:#fff;font-size:16px;font-weight:800;padding:16px;border-radius:14px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;opacity:${picking || detecting ? 0.6 : 1}`)} hover={picking || detecting ? {} : s('background:#2563EB')}>
      <Icon name="sparkles" size={20} color="#fff" />{picking || detecting ? 'Un attimo…' : 'Trasforma questo momento'}
    </Box>
  );

  const Timeline = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, background: '#f6f4f0', border: '1px solid #ece9e2', borderRadius: 12, padding: '10px 12px' }}>
      ...
      <div ref={scrollRef} style={{ overflowX: zoom > 1 ? 'auto' : 'hidden', overflowY: 'hidden', WebkitOverflowScrolling: 'touch' }}>
        <div ref={trackRef} onPointerDown={scrub} style={{ position: 'relative', width: `${zoom * 100}%`, minWidth: '100%', height: 76, borderRadius: 8, overflow: 'hidden', cursor: 'pointer', touchAction: 'none', background: '#e4e1da', display: 'flex', border: '1px solid #d8d4cb', transition: 'width .12s ease', willChange: 'width' }}>
          {strip.map((f, i) => (
          ...
          {segsSorted.map((seg) => duration ? (
            <div key={seg.id} onPointerDown={dragSeg(seg, 'move')} title="Trascina per spostare" style={{ position: 'absolute', top: 0, bottom: 0, left: `${(seg.tStart / duration) * 100}%`, width: `${Math.max(1, ((seg.tEnd - seg.tStart) / duration) * 100)}%`, background: 'rgba(59,131,246,.30)', border: `2px solid ${ACCENT}`, boxSizing: 'border-box', cursor: 'grab', touchAction: 'none', zIndex: 2 }}>
              <div onPointerDown={dragSeg(seg, 'start')} title="Inizio" style={{ position: 'absolute', left: -9, top: -2, bottom: -2, width: 18, background: ACCENT, borderRadius: 5, cursor: 'ew-resize', display: 'flex', alignItems: 'center', justifyContent: 'center', touchAction: 'none' }}><span style={{ width: 3, height: 28, background: '#fff', borderRadius: 2 }} /></div>
              <div onPointerDown={dragSeg(seg, 'end')} title="Fine" style={{ position: 'absolute', right: -9, top: -2, bottom: -2, width: 18, background: ACCENT, borderRadius: 5, cursor: 'ew-resize', display: 'flex', alignItems: 'center', justifyContent: 'center', touchAction: 'none' }}><span style={{ width: 3, height: 28, background: '#fff', borderRadius: 2 }} /></div>
            </div>
          ) : null)}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <button onClick={togglePlay} aria-label={playing ? 'Pausa' : 'Play'} style={{ border: 'none', background: 'transparent', cursor: 'pointer', display: 'flex', flex: 'none', padding: 0 }}>
          {playing
            ...
            : <svg width="18" height="18" viewBox="0 0 24 24" fill="#211f1c" style={{ marginLeft: 2 }}><polygon points="6 3 20 12 6 21 6 3" /></svg>}
        </button>
        <span style={{ fontSize: 12, fontWeight: 700, color: '#211f1c', fontVariantNumeric: 'tabular-nums', flex: 'none' }}>{clk(playhead)}</span>
        <div style={{ flex: 1 }} />
        {/* zoom con slider */}
        <Icon name="search" size={14} color="#8c867d" />
        <span style={{ fontSize: 12, fontWeight: 800, color: '#57534c', flex: 'none' }}>Zoom</span>
        <input className="gnm-zoom-range" type="range" min={1} max={8} step={0.1} value={zoom} onChange={e => setZoom(Number(e.target.value))} aria-label="Zoom timeline" style={{ flex: '0 1 250px', minWidth: 90 }} />
        <span style={{ fontSize: 12, fontWeight: 800, color: '#57534c', minWidth: 30, textAlign: 'center', flex: 'none' }}>{zoom.toFixed(1)}x</span>
        <div style={{ flex: 1 }} />
        <span style={{ fontSize: 12, fontWeight: 700, color: '#8c867d', fontVariantNumeric: 'tabular-nums', flex: 'none' }}>{clk(duration)}</span>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ background: '#eef4fe', border: '1px solid #cfe0fb', borderRadius: 14, padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={s('font-size:15px;font-weight:800;color:#1d5fd0')}>Come funziona</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fff', border: '1px solid #cfe0fb', borderRadius: 10, padding: '10px 12px' }}>
          <Icon name="film" size={16} color="#1d5fd0" />
          <span style={s('font-size:13px;color:#1d5fd0;font-weight:600')}>Ogni taglio usa <b>1</b> dei tuoi crediti video.{segments.length > 0 ? ` Crediti video usati: ${segments.length}` : ''}</span>
        </div>
      </div>
      <div className="max-md:!grid-cols-1" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 0.72fr) minmax(0, 1.28fr)', gap: 20, alignItems: 'start' }}>
      {/* COLONNA SX: video grande (sticky) + pulsante + barra tagli */}
      <div className="max-md:!static" style={{ position: 'sticky', top: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div style={{ position: 'relative', borderRadius: 14, overflow: 'hidden', background: '#000', display: 'flex', justifyContent: 'center', alignItems: 'center', aspectRatio: '9 / 16', maxHeight: '74vh' }}>
      ...
      {Timeline}
      {TransformBtn}
      <span style={s('font-size:12.5px;color:#8c867d;text-align:center;display:block')}>Metti in pausa all&apos;inizio della parte da togliere, poi premi qui.</span>
      <div style={{ height: 1, background: '#ece9e2', margin: '4px 0' }} />

      {/* Rilevamento automatico durata */}
      {detecting && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: 14, border: `1.5px solid #cfe0fb`, background: '#eef4fe', borderRadius: 12 }}>
          <span style={{ width: 18, height: 18, border: '3px solid #cfe0fb', borderTopColor: ACCENT, borderRadius: '50%', display: 'inline-block', animation: 'export-spin .8s linear infinite' }} />
          <span style={s('font-size:13px;font-weight:700;color:#1d5fd0')}>Sto calcolando quanto dura la parte da togliere…</span>
        </div>
      )}

      {/* Card per ogni taglio */}
      {segsSorted.length === 0 ? (
        <div style={s('text-align:center;padding:22px;border:1.5px dashed #d8d4cb;border-radius:12px;color:#8c867d;font-size:13.5px')}>
          Nessun taglio ancora. Guarda il video e premi <b>Trasforma questo momento</b>.
        </div>
      ) : [...segments].reverse().map((seg) => (
        <div key={seg.id} style={{ border: '1px solid #ece9e2', borderRadius: 14, padding: 14, background: '#fff', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
            <div style={s('font-size:14px;font-weight:800')}>Taglio {segments.indexOf(seg) + 1} <span style={{ color: '#b3aca1', fontWeight: 700 }}>· tolgo {(seg.tEnd - seg.tStart).toFixed(1)}s</span></div>
            <Box as="button" onClick={() => remove(seg.id)} style={s('flex:none;border:1px solid #f3d4d4;background:#fff;color:#dc2626;font-size:13px;font-weight:700;padding:8px 12px;border-radius:10px;cursor:pointer;display:inline-flex;align-items:center;gap:6px')} hover={s('background:#fef2f2')}>
              <Icon name="trash-2" size={15} color="#dc2626" />Rimuovi
            </Box>
          </div>
          {/* Stile (stesso look di Foto AI) */}
          <div style={{ fontSize: 11, fontWeight: 700, color: '#b3aca1', textTransform: 'uppercase', letterSpacing: '.06em' }}>Stile</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(96px, 1fr))', gap: 10 }}>
            {AI_STAGING_STYLES.map(st => {
              const sel = seg.styleId === st.id;
              return (
                <div key={st.id} onClick={() => update(seg.id, { styleId: st.id })} title={st.desc} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '14px 4px', borderRadius: 12, cursor: 'pointer', textAlign: 'center', border: sel ? '1.5px solid #3B83F6' : '1.5px solid transparent', background: sel ? '#eff6ff' : '#f8f7f5', boxShadow: sel ? '0 4px 12px rgba(59,131,246,0.12)' : 'none', transition: 'all .2s cubic-bezier(.4,0,.2,1)', position: 'relative' }}>
                  <span style={{ width: 24, height: 24, display: 'flex', color: sel ? '#1d5fd0' : '#57534c' }} dangerouslySetInnerHTML={{ __html: st.icon }} />
