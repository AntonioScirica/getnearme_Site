...
// Render i post feed SUBITO (slide_data → immagini → generated_content ped), così
// publish-ped ha qualcosa da pubblicare senza aspettare un cron separato. novideo:
// i video li rende il blocco sotto, non ri-triggerarli da generate-ped.
if (!videosOnly && total > 0) {
  console.log('\nRendering feed posts (generate-ped)...');
  const { default: generatePed } = await import('../src/lib/social/cron/generate-ped.js');
  for (const date of dates) {
    let out = null;
    const req = { query: { date, sync: '1', novideo: '1', account: accountId }, headers: { 'x-cron-secret': process.env.CRON_SECRET || '' } };
    const res = { setHeader() {}, status() { return this; }, json(o) { out = o; return o; } };
    try { await generatePed(req, res); console.log(`  ${date}: ${out?.generated ?? '?'} post feed`); }
    catch (e) { console.log(`  ${date}: render FAIL ${e.message}`); }
  }
}
